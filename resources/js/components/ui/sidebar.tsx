    ...
    ({ className, onClick, ...props }, ref) => {
        ...
        return (
            <Button
                ...
                variant="ghost"
                size="icon"
                // No h-7 override: `icon` is already the scale's 36px square, and shrinking the one
                // control that opens navigation on a phone to 28px made it the smallest tap target
                // in the app.
                className={className}
                onClick={(event) => {
                    onClick?.(event);
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<'main'>>(({ className, ...props }, ref) => {
    return (
        <main
            ref={ref}
            className={cn(
                // min-w-0 is load-bearing, not tidying. This is a flex item, so it defaults to
                // min-width:auto and cannot shrink below its widest descendant - which on the admin
                // screens is a nine-column table. The whole shell was then 1212px inside a 390px
                // phone: the page scrolled sideways, the header ran off the screen, and the
                // overflow-x-auto around the table never engaged because the table always fitted.
...
const SidebarInput = React.forwardRef<React.ElementRef<typeof Input>, React.ComponentProps<typeof Input>>(({ className, ...props }, ref) => {
    return (
        <Input
            ref={ref}
            data-sidebar="input"
            className={cn('h-8 w-full bg-background shadow-none focus-visible:ring-1 focus-visible:ring-sidebar-ring', className)}
            {...props}
        />
    );
});
    ...
    ({ className, asChild = false, ...props }, ref) => {
        ...
        return (
            <Comp
                ...
                className={cn(
                    'flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-hidden ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear focus-visible:ring-1 [&>svg]:size-4 [&>svg]:shrink-0',
