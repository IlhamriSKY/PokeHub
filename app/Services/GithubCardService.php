...
{
    /**
     * How long a second visitor waits on the first one's generation before being told to come back.
     *
     * Shorter than CardCapture's, and deliberately shorter than a slow completion: this is someone
     * who typed a name into a form and is watching it, where a capture is a README embed being
     * fetched by a crawler that nobody is waiting on. Holding a worker for the full two minutes to
     * spare them one retry is the wrong trade in both directions.
     */
    ...
    public function ensureProfile(string $login): array
    {
        ...
        $row = $this->cached($login);
        if ($row !== null) {
            return [$row, null, 200];
        }

        /*
         * One generation per login, however many people ask for the same handle at once.
         *
         * Everything past here is two GitHub round trips and a paid AI completion - the most
         * expensive thing this app does, on its only unauthenticated write path. The `card-generate`
         * limiter does not cover it, because that counts per IP and the case that hurts is one
         * handle trending somewhere: a hundred different visitors asking for the same uncached card
         * inside the same few seconds, each paying for its own completion and all but one of them
         * thrown away by the updateOrCreate below. CardCapture and AvatarCache already stand behind
         * this same lock; this was the one cache that did not.
         *
         * Waiters re-test the cache the moment they get in, so a second arrival costs the wait and
         * nothing else. The TTL clears the AI timeout, so the lock cannot expire under its holder
         * and let a second completion through.
         */
        try {
            return Cache::lock("card-generate:{$login}", (int) ($this->cfg['ai']['timeout'] ?? 120) + 60)
                ->block(self::LOCK_WAIT, function () use ($login) {
                    $row = $this->cached($login);

                    return $row !== null ? [$row, null, 200] : $this->generate($login);
                });
        } catch (LockTimeoutException) {
            // Still being generated by whoever got here first. A row that landed while we waited
            // beats an error; otherwise say so plainly rather than starting a second completion.
            $row = $this->cached($login);

            return $row !== null
                ? [$row, null, 200]
                : [null, 'That card is being generated right now. Try again in a moment.', 429];
        }
    }

    /**
     * An already-generated row for a login, or null when there is nothing usable stored.
     *
     * split() rather than github_json, so a legacy payload-only row still counts as a hit.
     */
    private function cached(string $login): ?Profile
    {
        $row = Profile::find($login);
        [$github] = $row?->split() ?? [null];

        return is_array($github) && ! empty($github['login']) ? $row : null;
    }

    /**
     * Fetch a profile, write it and return it. Only ever called with that login's lock held.
     *
     * @return array{0: ?Profile, 1: ?string, 2: int} [row, errorMessage, status]
     */
    private function generate(string $login): array
    {
